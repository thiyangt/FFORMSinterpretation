...
different forecast model, and ii) local explanation of feature
...
evaluate test set error. We use a vote matrix calculated based on OOB
observations as a model-diagnostic tool. The vote matrix
...
The vote matrix (\(N \times P\); \(N\) is total number of observations;
\(P\) is number of classes) contains the proportion of times each
observation was classified to each class based on OOB sample. This
information for the yearly data is presented in \autoref{fig:yearlyoob}.
This is an alternative way of visualizing the vote-matrix information.
The other way of representing vote matrix involves ternary plot
...
differenced series do not interact with other features heavily within
...
heat matrices of Friedman's H-Statistic presented that a substantial
...
features are more important on its own. The features involve in
...
category all or a subset of ACF/PACF-based features interact with each
other. This confirm that information regarding correlation structure of
the time series is an essential information for the choice of model
selection.
