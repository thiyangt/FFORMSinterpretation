...
**Local Interpretable Model-agnostic Explanations: quarterly series**
...
According to \autoref{fig:oobdaily} the distributions corresponds to observations that have been correctly classified dominate the top, indicating a fairly good classification from FFORMS framework. However, there are few observations that have been incorrectly classified to tbats class with very high probabilities. In general, neural network models have higher chance of being selected for daily time series. 
...
Variable importance graph for daily data is shown in \autoref{fig:vidaily}. Overall, similar to the previous results most important features for determining suitable forecast-models for daily time series are, strength of seasonality corresponds to the weekly seasonality (7), stability, trend, lumpiness and linearity. Further more, length of the series is important in determining random walk, random walk with drift, mstlarima, mstlets, stlar, theta and nn classes. Strength of seasonality corresponds to the annual seasonality (365.25) is appeared to be important in determining the selection of theta models. \autoref{fig:dailypdp} shows the partial dependency plots of the top 3 features from the FFORMS framework. According to the results of \autoref{fig:dailypdp}  shorter series tends to select random walk with drift models while probability of selecting tbats, and theta models increases as the length of series increases. Neural network models shows a non-monotonic relationship with length of the series (N). The theta models tends to be selected for series with high annual seasonality but very low weekly seasonality.
...
The proportion of times each time series was classified to each class for hourly series are shown in \autoref{fig:hourlyoob}. Except neural network class, the distribution corresponds to the correct classification dominated the top. It is important to note that, in neural network category, distribution corresponds to snaive class slightly dominated the distribution corresponds to neural network category. Overall, for hourly series random walk with drift models, tbats and neural network models have high chance of being selected. Furthermore, all distributions corresponds to the neural network category located further away from zero with respect to  x-axis indicates, every time series selects neural network models at least once from the trees in the random forest. In general, strength of daily seasonality (period=24) appear to be more important than than the strength of weekly seasonality (period=168).  Furthermore, entropy, linearity, sum of squares of first 5 coefficients of PACF, curvature, trend, spikiness and stability were found to be the most important features in determining best forecasting method for hourly time series. Although the length of the series found to be one of the most important feature from previous results, only snaive category ranked it among top 5 for hourly time series. The strength of seasonality corresponds to weekly seasonality also seems to be one of the most important feature for the classes snaive, random walk, mstlarima, and tbats. The partial dependency plots shows that probability of selecting random walk, random walk with drift, theta model and white noise process decreased with higher strength of daily seasonality, while the  opposite relationship hold for other classes.On the other hand, probability of selecting random walk model increased with the increase in strength of weekly seasonality. 


```{r hourlyoob, fig.height=10, fig.width=10, fig.cap="Distribution of proportion of times each hourly time series was classified to each class in the forest.  Each row represent the predicted class label and colour of boxplots corresponds to true class label. There are ten rows in the plot corresponds to each predicted class represented by Y-axis. X-axis denotes the proportion of times a time series is classified in each class. On each row, the distribution of correct classification dominated the top.", fig.pos="h", cache=TRUE}
```

\newpage

```{r vihourly, fig.height=10, fig.width=10, fig.cap="Feature importance plot hourly series. Longer bars indicate more important features. Top 5 features are highlighted in red.", fig.pos="h"}
```
...
Forecast model selection is both time and compute cost intensitive process. Consequently, the application of machine-learning approaches to predict suitable forecasting model from large number of potentially relevant time series features is a topic gowing popularity in the field of time series forecasting. In this paper we used machine learning interpretability methods to investigate  what is happening under the hood of these trained models.
Most important features were identified using different variable importance measures. Our motivation for this chiose was to draw more general conclusions that would be possible from conducting this investigation by means of 

The degree of relevance of the respective input featues are assessed globally and locally. Understand the entire model that you've trained on a global scale and also to zoom into local regions of your data. These influence measures may or may not reflect the usefulness of individual predictor in the absence of others. For example, a predictor on which the target function has no dependence at all may be useful if it is highly corelated with an important predictor and the latter is removed from the ensemble. 

Trend, strength of seasonality, linearity, are noteworthy among the top 10 features withing each frequency category. Features that frequently appear are more relevant than those that tend to appear only in less influential predictors. This is consistent with the previous findings. For example, studies such a... have reported that .. is associated with the . Further, indeed there is a strong relationship with length of the time series and  


Partial dependency plots visualized the learned relationship between features and predictions. Ans the results show that they match with our expectations with leades to the trustability of the model. Partial dependency plots enhance the understanding of the features with prediction outcome. THey can also enhance the trust when the displayed relationship confirm to domain knowledge expectations. 

Probability f selecting neural network models decreases with trend

It is apparent from the heat marices of Friedman's H-statistic presented that substantial interaction effect existed. The results of this study help us to explore and communicate the nature of the contribution of features to the choices of FFORMS framework.cThis shows how different features affect the choice of forecast model selection. Furthur the results of this study ae useful in identifying new ways to of improving forecast accuracy. The research could be extended in a number of different directions, including the use of more complex multivariae models to take into account .... 
...
```{r yearlypca, fig.height=10, fig.width=20, fig.cap="Distribution of prediction space across instance space for yearly time series. PC1 and PC2 are the first two principal components.", fig.pos="h", fig.align="center", cache=TRUE}
```

\clearpage

```{r quarterlypca, fig.height=15, fig.width=20, fig.cap="Distribution of prediction space across instance space for quarterly time series. PC1 and PC2 are the first two principal components.", fig.pos="h", cache=TRUE}
```

\newpage

```{r monthlypca, fig.height=15, fig.width=20, fig.cap="Distribution of prediction space across instance space for monthly time series. PC1 and PC2 are the first two principal components.", fig.pos="h", cache=TRUE}
```

\clearpage

```{r pcaweekly, fig.height=15, fig.width=20, fig.cap="Distribution of prediction space across instance space for weekly time series. PC1 and PC2 are the first two principal components.", fig.pos="h", fig.align="center", cache=TRUE}
```

\clearpage

```{r pcadaily, fig.height=5, fig.width=10, fig.cap="Distribution of prediction space across instance space for daily time series. PC1 and PC2 are the first two principal components.", fig.pos="h", fig.align="center", cache=TRUE}
```


```{r pcahourly, fig.height=5, fig.width=10, fig.cap="Distribution of prediction space across instance space for hourly time series. PC1 and PC2 are the first two principal components.", fig.pos="h", fig.align="center", cache=TRUE}
```
...
```{r qpdptwo, fig.height=30, fig.width=10}
```
