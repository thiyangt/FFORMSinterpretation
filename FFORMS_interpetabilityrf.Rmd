...
```{r pdpyearly, fig.height=20, fig.width=15, fig.cap="Partial dependence plots for the top-three features get selected most within each class for yearly series. The shading shows the 95% confidence intervals. Y-axis denotes the probability of belong to corresponding class. All features show a nonlinear relationship with predicted probabilities.", fig.pos="h"}
```

\newpage

```{r friedmany, fig.height=8, fig.width=8, fig.cap="Heat maps of relative strength of all possible pairwise interactions calculated based on Friedman's H-statistic (yearly series). Strength of trend shows less interactivity with other features.", fig.pos="h", message=FALSE, warning=FALSE, dev='pdf'}
```
...
```{r pdpquarterlysec2, fig.height=28, fig.width=20, fig.cap="Partial dependence plots for the top-three features get selected most within each class inside quarterly and monthly FFORMS frameworks. Additionally, N is included to observe the effects stated in the literature. The shading shows the 95% confidence interval. Y-axis denotes the probability of belonging to corresponding class. Red colour is for PDP drawn based on quarterly data and blue colour is for the PDP drawn based on monthly data.(Continue from Figure 9)", fig.pos="h"}
```

\clearpage


```{r friedmanQM, fig.height=10, fig.width=10, fig.cap="Heat maps of relative strength of all possible pairwise interactions calculated based on Friedman's H-statistic for quarterly and monthly data.", fig.pos="h", message=FALSE, warning=FALSE, dev='pdf'}
```
...
\newpage
```{r friedmanHW, fig.height=7, fig.width=7, fig.cap="Heat maps of relative strength of all possible pairwise interactions calculated based on Friedman's H-statistic for weekly series.", fig.pos="h", message=FALSE, warning=FALSE, dev='pdf'}
```
